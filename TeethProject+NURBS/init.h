...
void set_curve(char* filename, SB* slice, REAL tx, REAL ty, REAL tz, INT mode)
{
	INT numVertex = 0;
	INT numFaces = 0;
	REAL x_max, xmax_y, x_min, xmin_y, y_max, ymax_x, y_min, ymin_x, x, y, z;
	Point3 start[4], middle[4], end[4], ctrlp_2[4], ctrlp_3[4];
	FILE* fp = NULL;
	char line[256];						//get line

	/* load model */
	std::string b = std::string(filename);
	std::string a = "src\\";
	a += b;
	...
	rewind(fp);							//back to start point of file
	slice->verticenum = numVertex;		//set slice's facenum and verticenum
	...
	while (!feof(fp)) {
		...
		if (line[0] == 'v' && line[1] == ' ') {
			...
			slice->faces[numFaces][X] = x1 - 1;					//면의 정점 
