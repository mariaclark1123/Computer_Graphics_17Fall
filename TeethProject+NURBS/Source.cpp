...
/* realmodel */
REAL vertices[10000][3];
INT faces[15000][3];
INT numFaces = 0, numVertex = 0;

//139_5
REAL tx = 240, ty = 460, tz = 50;

//20170801 2:11
//2_4
//REAL tx = -9, ty = 95, tz = 65;
//2_8
//REAL tx = -15, ty = 95, tz = 65;
//2_17 
//REAL tx = -22, ty = 78, tz = 38;
//3_3
//REAL tx = -5, ty = 97, tz = 60;
...
/*3 big surface(7*7) and 2 small surface (2*7)*/
float xarray[8][49];
float yarray[8][49];
float zarray[8][49];
float xb[2][28];
...
void lightm()
{
	...
	GLfloat lpos[4] = { pos_x,pos_y,pos_z,1.0f };
...
void getmodel(char* filename)
{
	...
	while (!feof(fp))
	{
		...
		if (line[0] == 'v' && line[1] == ' ')
		{
			...
			vertices[IdxVertex][X] = (x - 355) * 6 + 200;
...
void Init()
{
	...
	getmodel("src/15.txt");
	//	set_curve("15_242.txt", &slice[0], tx, ty, tz, 0);
	set_curve("15_247.txt", &slice[0], tx, ty, tz, 1);
	set_curve("15_252.txt", &slice[1], tx, ty, tz, 2);
	set_curve("15_257.txt", &slice[2], tx, ty, tz, 3);
	set_curve("15_262.txt", &slice[3], tx, ty, tz, 4);
	set_curve("15_267.txt", &slice[4], tx, ty, tz, 5);
	set_curve("15_272.txt", &slice[5], tx, ty, tz, 6);
	set_curve("15_277.txt", &slice[6], tx, ty, tz, 7);
	set_curve("15_282.txt", &slice[7], tx, ty, tz, 8);
	set_curve("15_287.txt", &slice[8], tx, ty, tz, 9);
	set_curve("15_292.txt", &slice[9], tx, ty, tz, 10);
	set_curve("15_297.txt", &slice[10], tx, ty, tz, 11);
	set_curve("15_302.txt", &slice[11], tx, ty, tz, 12);
	set_curve("15_307.txt", &slice[12], tx, ty, tz, 13);
	set_curve("15_312.txt", &slice[13], tx, ty, tz, 14);
	set_curve("15_317.txt", &slice[14], tx, ty, tz, 15);
	set_curve("15_322.txt", &slice[15], tx, ty, tz, 16);
	set_curve("15_327.txt", &slice[16], tx, ty, tz, 17);
	set_curve("15_332.txt", &slice[17], tx, ty, tz, 18);
	set_curve("15_337.txt", &slice[18], tx, ty, tz, 19);
	set_curve("15_342.txt", &slice[19], tx, ty, tz, 20);
	set_curve("15_347.txt", &slice[20], tx, ty, tz, 21);
	//set_curve("15_350.txt", &slice[22], tx, ty, tz, 22);

	/*20170823 save control pts to first x,y,z array*/
	for (int num = 0; num < 3; num++)
	{
		for (int j = 0; j < 7; j++)
		{
			...
			for (int m = 0; m < 4; m++)
			{
				xarray[num][j * 7 + m] = slice[num * 6 + j].cur[0].control_pts[m % 4][0];
				yarray[num][j * 7 + m] = slice[num * 6 + j].cur[0].control_pts[m % 4][1];
			}
			for (int m = 4; m < 7; m++)
			{
				xarray[num][j * 7 + m] = slice[num * 6 + j].cur[1].control_pts[m % 4 + 1][0];
				yarray[num][j * 7 + m] = slice[num * 6 + j].cur[1].control_pts[m % 4 + 1][1];
			}
		}
		for (int i = 0; i < 7; i++)
		{
			for (int m = 0; m < 7; m++)
				zarray[num][i * 7 + m] = -260 + 30 * i + num * 180;
		}
		/*index 3-5*/
		for (int j = 0; j < 7; j++)
		{
			...
			for (int m = 0; m < 4; m++)
			{
				xarray[4 + num][j * 7 + m] = slice[num * 6 + j].cur[2].control_pts[m % 4][0];
				yarray[4 + num][j * 7 + m] = slice[num * 6 + j].cur[2].control_pts[m % 4][1];
			}
			for (int m = 4; m < 7; m++)
			{
				xarray[4 + num][j * 7 + m] = slice[num * 6 + j].cur[3].control_pts[m % 4 + 1][0];
				yarray[4 + num][j * 7 + m] = slice[num * 6 + j].cur[3].control_pts[m % 4 + 1][1];
			}
		}
		/*no change except index*/
		for (int i = 0; i < 7; i++)
		{
			for (int m = 0; m < 7; m++)
				zarray[4 + num][i * 7 + m] = -260 + 30 * i + num * 180;
		}
	}
	/*for last one*/
	for (int j = 14; j < 21; j++)
	{
		for (int m = 0; m < 4; m++)
		{
			xarray[3][(j - 14) * 7 + m] = slice[j].cur[0].control_pts[m % 4][0];
			yarray[3][(j - 14) * 7 + m] = slice[j].cur[0].control_pts[m % 4][1];
		}
		for (int m = 4; m < 7; m++)
		{
			xarray[3][(j - 14) * 7 + m] = slice[j].cur[1].control_pts[m % 4 + 1][0];
			yarray[3][(j - 14) * 7 + m] = slice[j].cur[1].control_pts[m % 4 + 1][1];
		}
	}
	for (int i = 0; i < 7; i++)
	{
		for (int m = 0; m < 7; m++)
			zarray[3][i * 7 + m] = 30 * i + 160;
	}
	...
	for (int j = 14; j < 21; j++)
	{
		for (int m = 0; m < 4; m++)
		{
			xarray[7][(j - 14) * 7 + m] = slice[j].cur[2].control_pts[m % 4][0];
			yarray[7][(j - 14) * 7 + m] = slice[j].cur[2].control_pts[m % 4][1];
		}
		for (int m = 4; m < 7; m++)
		{
			xarray[7][(j - 14) * 7 + m] = slice[j].cur[3].control_pts[m % 4 + 1][0];
			yarray[7][(j - 14) * 7 + m] = slice[j].cur[3].control_pts[m % 4 + 1][1];
		}
	}
	for (int i = 0; i < 7; i++)
	{
		for (int m = 0; m < 7; m++)
			zarray[7][i * 7 + m] = 30 * i + 160;
	}

	/*set_curve("2_4_slice0.txt", &slice[0], tx, ty, tz);
	set_curve("2_4_slice1.txt", &slice[1], tx, ty, tz);
	set_curve("2_4_slice2.txt", &slice[2], tx, ty, tz);
	set_curve("2_4_slice3.txt", &slice[3], tx, ty, tz);
	set_curve("2_4_slice4.txt", &slice[4], tx, ty, tz);
	set_curve("2_4_slice5.txt", &slice[5], tx, ty, tz);
	set_curve("2_4_slice6.txt", &slice[6], tx, ty, tz);
	set_curve("2_4_slice7.txt", &slice[7], tx, ty, tz);
	set_curve("2_4_slice8.txt", &slice[8], tx, ty, tz);
	set_curve("2_4_slice9.txt", &slice[9], tx, ty, tz);*/

	/*set_curve("2_8_slice0.txt", &slice[0], tx, ty, tz);
	set_curve("2_8_slice1.txt", &slice[1], tx, ty, tz);
	set_curve("2_8_slice2.txt", &slice[2], tx, ty, tz);
	set_curve("2_8_slice3.txt", &slice[3], tx, ty, tz);
	set_curve("2_8_slice4.txt", &slice[4], tx, ty, tz);
	set_curve("2_8_slice5.txt", &slice[5], tx, ty, tz);
	set_curve("2_8_slice6.txt", &slice[6], tx, ty, tz);
	set_curve("2_8_slice7.txt", &slice[7], tx, ty, tz);
	set_curve("2_8_slice8.txt", &slice[8], tx, ty, tz);
	set_curve("2_8_slice9.txt", &slice[9], tx, ty, tz);
	set_curve("2_8_slice10.txt", &slice[10], tx, ty, tz);*/

	/*set_curve("2_18_slice0.txt", &slice[0], tx, ty, tz);
	set_curve("2_18_slice1.txt", &slice[1], tx, ty, tz);
	set_curve("2_18_slice2.txt", &slice[2], tx, ty, tz);
	set_curve("2_18_slice3.txt", &slice[3], tx, ty, tz);
	set_curve("2_18_slice4.txt", &slice[4], tx, ty, tz);
	set_curve("2_18_slice5.txt", &slice[5], tx, ty, tz);
	set_curve("2_18_slice6.txt", &slice[6], tx, ty, tz);
	set_curve("2_18_slice7.txt", &slice[7], tx, ty, tz);
	set_curve("2_18_slice8.txt", &slice[8], tx, ty, tz);
	set_curve("2_18_slice9.txt", &slice[9], tx, ty, tz);
	set_curve("2_18_slice10.txt", &slice[10], tx, ty, tz);*/

	/*set_curve("3_3_slice0.txt", &slice[0], tx, ty, tz);
	set_curve("3_3_slice1.txt", &slice[1], tx, ty, tz);
	set_curve("3_3_slice2.txt", &slice[2], tx, ty, tz);
	set_curve("3_3_slice3.txt", &slice[3], tx, ty, tz);
	set_curve("3_3_slice4.txt", &slice[4], tx, ty, tz);
	set_curve("3_3_slice5.txt", &slice[5], tx, ty, tz);
	set_curve("3_3_slice6.txt", &slice[6], tx, ty, tz);
	set_curve("3_3_slice7.txt", &slice[7], tx, ty, tz);
	set_curve("3_3_slice8.txt", &slice[8], tx, ty, tz);*/
}
...
void Showslice()
{
	...
	for (INT i = 0; i < slice[mode].facenum - 1; i++)
	{
		...
		REAL p1[3] = { slice[mode].vertices[f1][X], slice[mode].vertices[f1][Y] };
		REAL p2[3] = { slice[mode].vertices[f2][X], slice[mode].vertices[f2][Y] };
		REAL p3[3] = { slice[mode].vertices[f3][X], slice[mode].vertices[f3][Y] };
...
void display_callback(void)
{
	...
	for (int i = 0; i < 8; i++)
	{
		NurbsFace(2, 9, u, 30, 9, v, 30, xarray[i], yarray[i], zarray[i], w, suv);
		if (showsurface)
			ShowSurface(30, 30, suv, 0);
		if (showmodel)
			ShowSurface(30, 30, suv, 1);
	}
...
void keyboard_callback(unsigned char key, INT x, INT y)
{
	...
	case 'a':
	{
		...
		fprintf(fp, "slice[%d].cur 1: (%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f)\n", mode, slice[mode].cur[0].control_pts[0][0], slice[mode].cur[0].control_pts[0][1], slice[mode].cur[0].control_pts[1][0], slice[mode].cur[0].control_pts[1][1], slice[mode].cur[0].control_pts[2][0], slice[mode].cur[0].control_pts[2][1], slice[mode].cur[0].control_pts[3][0], slice[mode].cur[0].control_pts[3][1]);
		fprintf(fp, "slice[%d].cur 2: (%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f)\n", mode, slice[mode].cur[1].control_pts[0][0], slice[mode].cur[1].control_pts[0][1], slice[mode].cur[1].control_pts[1][0], slice[mode].cur[1].control_pts[1][1], slice[mode].cur[1].control_pts[2][0], slice[mode].cur[1].control_pts[2][1], slice[mode].cur[1].control_pts[3][0], slice[mode].cur[1].control_pts[3][1]);
		fprintf(fp, "slice[%d].cur 3: (%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f)\n", mode, slice[mode].cur[2].control_pts[0][0], slice[mode].cur[2].control_pts[0][1], slice[mode].cur[2].control_pts[1][0], slice[mode].cur[2].control_pts[1][1], slice[mode].cur[2].control_pts[2][0], slice[mode].cur[2].control_pts[2][1], slice[mode].cur[2].control_pts[3][0], slice[mode].cur[2].control_pts[3][1]);
		fprintf(fp, "slice[%d].cur 4: (%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f),(%.2f,%.2f)\n", mode, slice[mode].cur[3].control_pts[0][0], slice[mode].cur[3].control_pts[0][1], slice[mode].cur[3].control_pts[1][0], slice[mode].cur[3].control_pts[1][1], slice[mode].cur[3].control_pts[2][0], slice[mode].cur[3].control_pts[2][1], slice[mode].cur[3].control_pts[3][0], slice[mode].cur[3].control_pts[3][1]);
		fprintf(fp, "----------------------------------------------------------------------------\n");
...
{
	if ((fp = fopen("src\info.txt", "w")) == NULL)
	{
